...
def execute_callback() -> None:
    ...
    start creates a run and continues until the run stops.
    continue resumes a run and does the same. It does not reset the attempt count.
    stage dispatches the next eligible stage once. It takes no stage id.
    On a new run, pass --workflow and --task instead of RUN.

    --prior is the completed run named by requires. It is required on create
    ...
    Stdout is one JSON object: run_id, run_status, stop_reason, outputs, stages.
    Progress is on stderr.

    Exit 0 when the run is completed, or a one-shot stage outcome is complete.
    Exit 1 when the run is blocked, or a one-shot stage is not complete.
    Exit 2 on a configuration error.
    Exit 3 when start or continue spends --attempts. The run stays active.
...
def execute_start(
    ...
    task: Annotated[
        str,
        typer.Option("--task", metavar="TASK", help="Task recorded on the new run."),
    ],
    prior: Annotated[
        Optional[str],
        typer.Option("--prior", metavar="RUN", help=_PRIOR_HELP, show_default=False),
    ] = None,
    attempts: Annotated[
        int,
        typer.Option(
    ...
    """Create a run and continue until it stops.

    WORKFLOW is the workflow id. --task is recorded on the new run.
    """
    raise typer.Exit(
        execute_workflow(
...
def execute_continue(
    run: Annotated[str, typer.Argument(metavar="RUN", help="Run to resume.")],
    prior: Annotated[
        Optional[str],
        typer.Option("--prior", metavar="RUN", help=_PRIOR_HELP, show_default=False),
    ] = None,
    ...
    """Resume a run until it stops.

    A completed, blocked, or cancelled run is not dispatched again.
    """
    raise typer.Exit(
        execute_workflow(
            find_workspace(invocation_cwd()),
...
def execute_stage(
    ...
    Pass RUN, or both --workflow and --task. Not both forms.
    """
    has_run = _text(run) is not None
    has_workflow = _text(workflow) is not None
    has_task = _text(task) is not None
    if has_run and (has_workflow or has_task):
        raise typer.BadParameter("Pass RUN, or --workflow and --task, not both.")
    if not has_run and not (has_workflow and has_task):
        raise typer.BadParameter("Pass RUN, or both --workflow and --task.")
    raise typer.Exit(
        execute_workflow(
            find_workspace(invocation_cwd()),
            action="stage",
