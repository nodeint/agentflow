...
def execute_named_workflow(
    ...
    *,
    ...
    if stage_id is None and (max_attempts is None or max_attempts < 1):
    ...
    if run_id:
        ...
        if not request.workflow_id:
            raise ConfigurationError("Creating a run requires --workflow-id.")
        if not request.task:
...
def _run_execute_once(
    ...
    _print_execute_final(status, stop_reason)
...
def _run_execute_loop(
    ...
    while True:
        ...
        if failed_stage_count(snapshots, selected.stage_id) >= max_attempts:
            _print_execute_final(status, "retry_exhausted")
            return 3
        _dispatch_execute_stage(
            workspace,
            workflow,
...
def _bind_execute_run(
    ...
    if workflow_id and workflow_id != stored_workflow:
        raise ConfigurationError(
            f"--workflow-id {workflow_id} does not match run {stored_workflow}."
        )
...
def _print_execute_final(status: Dict[str, Any], stop_reason: str) -> None:
